...
def validate_request(instruction, seed, controller, bottle_start, camera):
    ...
    if controller not in ('learned', 'learned_visual', 'learned_wide', 'programmed'):
        raise CommandError('Choose one of the listed controllers.')
    if bottle_start not in ('upright', 'wide_left', 'wide_rectangle') or camera not in CAMERAS:
...
def trial_report(task, seed, controller, bottle_start, started):
    ...
    return {
        ...
        'control_inputs': ('Live RGB for bottle approach/grasp, and late mug correction during table setting; motor feedback and a separate physical stop/score monitor.'
            if controller=='learned_wide' else 'Initial RGB views plus live stereo correction during mug placement; motor feedback and a separate physical stop/score monitor.'
        ...
        'destination_xy_m': snapshot.get('destination_xy_m'),
        'completed_route_legs': snapshot.get('completed_route_legs'),
    }
...
def run_trial(instruction, seed=42, controller='learned', bottle_start='upright',
    ...
    try:
        ...
        if controller in ('learned_visual','learned_wide') and policy_factory is not None:
            raise CommandError('Live visual control uses the verified OpenVINO CPU controller.')
        ...
        if bottle_start == 'wide_left':
            ...
            from .wide_bottle_task import wide_bottle_pose
            address = model.joint('bottle_free').qposadr[0]
            data.qpos[address:address+7] = wide_bottle_pose(seed, layout['table_z'])
        ...
        data.time = 0.
        from .trial_preview import TrialPreview
        renderer = TrialPreview(xml)

        def frame():
            return renderer.frame(data, camera)

        yield frame(), 'Reading the instruction and preparing a fresh scene…', {'status': 'preparing', 'seed': seed}
        if controller == 'learned_wide':
            from .wide_bottle_task import make_sequence
            task = make_sequence(model, data, layout, plan)
        elif controller in ('learned','learned_visual'):
