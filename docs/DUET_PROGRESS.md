...
## 2026-09-16 — SmolVLA fine-tune (v2), live demo dashboard, mug reposition

- Collected `datasets/duet-micro-v1` (50 episodes, 5 skills × 10 seeds 20261001-20261010), converted to LeRobot v3 format (`scripts/prepare_lerobot_dataset.py`), pushed to `pjmorales04/duet-micro-v1` (HF dataset, confirmed via Hub commit log: "raw duet-micro-v1 collection output (50 episodes, 5 skills x 10 seeds)").
- `duet-smolvla-v1`: fine-tuned `lerobot/smolvla_base` for 1000 steps, batch 16, on Colab (T4/A100). Loads cleanly (0 missing/unexpected keys against `SmolVLAConfig`/`SmolVLAPolicy`). Live rollout quality poor — e.g. seed 20261001 bottle-placement distance 22.6cm vs 2.6cm tolerance.
- `duet-smolvla-v2`: re-trained 7000 steps, batch 32, no AMP (bf16 GradScaler incompatibility on A100 forced dropping `--policy.use_amp`), `--save_freq=1000`. Pushed to `pjmorales04/duet-smolvla-v2`, pulled down and verified 0 missing/unexpected keys. Wired into `simulation_lab/vla_task.py`'s `CHECKPOINT`.
- Live seed evaluation (`api/command` mode=`learned_vla`, bottle skill, all 10 seeds, position measured against `/api/state` targets, not the VLA task's own always-"succeeded" status): seed `20261009` passes (1.21cm), seed `20261001` close but fails (6.39cm), remaining 8 seeds 25-27cm off. `20261009` is the recommended seed for the VLA demo tab going forward.
- CPU inference optimizations (`simulation_lab/vla_task.py`): `torch.set_num_threads(cpu_count-2)`, flow-matching `num_steps` reduced 10→5. Dynamic INT8 quantization attempted and reverted — VLM backbone's `q_proj` breaks under quantized-linear dtype expectations.
- Server preloads the SmolVLA checkpoint at startup (`simulation_lab/server.py`) — the module-level `_CACHE` in `vla_task.py` is otherwise cold-loaded on the first live request, which raced `engine.submit`'s 15s command timeout and failed the first click every time.
- Demo dashboard (`simulation_lab/web/demo.html/.js/.css`) rebuilt: live seed-list + progress bar + right/left arm function boxes (split by each object's live spawn side, not hardcoded) for both Expert and VLA tabs; HD gallery re-render (`scripts/render_hd_gallery.py`, re-simulates from saved `trajectory.npz`/`initial-integration-state.npy` at 1280x960 instead of upscaling the 320x240 collection-time frames); honest VLA-tab disclaimer with real training details.
- Mug final target moved beside the bottle: cherry-picked `master`'s `0d79dff` intent (`config/dinner-layout.json` mug `[0.250,-0.058]` → `[0.24,0.02]`), then revised again to `[0.15,0.05]` (closer, more clearly "beside" the bottle target `[0.152,0.126]`, clearance-checked against both plate and bottle placement radii — not yet visually re-rendered/confirmed, per explicit instruction to hold rendering while a mug-pour feature is in progress). `tests/test_dinner_layout.py::test_mug_final_position_is_beside_bottle` updated to check distance-to-bottle (0.059m-0.12m) instead of the old fixed-offset assertion.
